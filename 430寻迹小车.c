...
#define left_reversal       P1OUT=0X02;      //3轮反转
#define right_reversal      P1OUT=0X20;      //4轮反转
#define back_both           P1OUT=0X02+0X20;      //3,4反转
#define straight_both       P1OUT=0X00;      //3,4正传

/**********************************************************
*P1.2控制OUT3、4 P1.3控制OUT1、2
*P1.0 P1.1控制3、4正反转 P1.4、P1.5控制1、2正反转
*光电模块：1左微 2右微 3左 4右
*分布：                *1     *2问
...
void system(void)                       //I/O端口定义
{
     ...
     straight_both;                //设置正反转
     //P1OUT|=0X08;                    //设置P1.3起始为低点平
     //_BIS_SR(LPM0_bits);            //进入低功耗模式
...
char test3(void)                         //对3进行检测
{
  if(P02IN&0x08==0x8)                     //若检测到黑线 返回 1
    return 1;
  return 0;
}

char test4(void)                         //对4进行检测
{
  if(P2IN&0x10==0x10)                     //若检测到黑线 返回 1
    return 1;
  return 0;
}

char test_12(void)                         //对1 2进行检测
{
  if(P2IN&0x06!=0x00)                     //若至少有一个检测到黑线 返回 1
    return 1;
  return 0;
}
...
int main( void )
{
  ...
  diff[0]=1;diff[1]=3;diff[2]=1;
  while(1)
  { 
    char j;
     //delay_ms(40);
     //if(i==0x00)               //当3,4检测到白线时进行微调
    // {
         j=P2IN&0X06;
         switch(j)                  //只检查微调控制1 2
         { 
         ...
         case 0x06:go_straight();  break;          //都高直走
         }
         //LCD1602_display(diff);
    // }
    /* if(i==0x10)              //当4检测到黑线，右h转
     {
       right();
       delay_ms(10);
        while(!test1_2());
     }
     if(i==0x08)              //当3检测到黑线，左转
     {
       left();
       delay_ms(10);
       while(!test1_2());
     }
     if(i==0x18)              //当3，4都检测到黑线，直走
     {
       go_straight();
       delay_ms(10);
       while(!test1_2());
...
__interrupt void Port_2(void)
{
  ...
  delay_ms(3);
  straight_both;
  //delay_us(800);                            //这里延时给小车向前跑一段距离再来判断状态
    ...
    else                                          //丁字路口（第二种情况）
    {
      switch(diff[1])
      {
        ...
        case 4:
             ...
             /*while((P2IN&0X18)!=0X08)
             {
               j=P2IN&0X06;
                switch(j)                  //只检查微调控制1 2
               { 
                case 0x04:PWM1(65);PWM2(30); break;          //2高 右微调
                case 0x02:PWM1(30);PWM2(65);  break;          //1高 左微调
                case 0x00:slow();         break;          //都低停止
                case 0x06:PWM1(70);PWM2(70);  break;          //都高直走
               }
             }*/
              //left_higher();
             //delay_ms(10);
